...
export default function ResetPassword() {
  ...
  useEffect(() => {
    if (!verified || !email || !otp) {
      setError('Invalid access. Please start the password reset process from the beginning.');
    }
  }, [verified, email, otp]);
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-center">
        <div className="max-w-md w-full bg-white rounded-3xl p-10 shadow-xl border border-slate-100">
          ...
          <Link to="/login" className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white rounded-2xl font-bold shadow-lg shadow-brand-600/20 transition-all flex items-center justify-center gap-2">
            <span>Log In Now</span><ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </div>
    );
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={loading || !verified}
              ...
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <span>Reset Password</span>}
            </button>
          </form>
        </div>
      </div>
